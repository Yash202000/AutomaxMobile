          ...
          <View style={styles.titleCardContent}>
            ...
            {request.current_state && (
              <View style={styles.statusContainer}>
                ...
                <Text style={[styles.statusText, { color: COLORS.accent }]}>{request.current_state.name}</Text>
              </View>
            )}
          </View>
        </View>
        ...
        <View style={[styles.card, { marginBottom: availableTransitions.length > 0 ? 100 : 30 }]}>
          ...
          {request.transition_history && request.transition_history.length > 0 ? (
            <View style={styles.timeline}>
              {request.transition_history.map((item: any, index: number) => (
                <View key={item.id} style={styles.timelineItem}>
                  <View style={styles.timelineLeft}>
                    <View style={[styles.timelineDot, { backgroundColor: COLORS.accent }]} />
                    {index < request.transition_history.length - 1 && <View style={styles.timelineLine} />}
                  </View>
          ...
          onPress={() => router.push({
            ...
            params: {
              ...
              incident: JSON.stringify({
                id: request.id,
                classification_id: request.classification_id || request.classification?.id,
                location_id: request.location_id || request.location?.id,
                department_id: request.department_id || request.department?.id,
                assignee_id: request.assignee_id || request.assignee?.id,
...
const styles = StyleSheet.create({
  ...
  requestTitle: { fontSize: 18, fontWeight: 'bold', color: COLORS.text.primary, marginBottom: 4 },
  ...
  transitionBadges: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 6 },
  fromBadge: { backgroundColor: '#FEE2E2', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6, flexShrink: 1 },
  fromBadgeText: { fontSize: 12, color: COLORS.error, fontWeight: '600' },
  toBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6, flexShrink: 1 },
