      ...
      <View style={styles.cardContent}>
        <View style={styles.cardHeader}>
          <View style={styles.idContainer}>
            ...
            <Text style={styles.idText}>{request.incident_number}</Text>
          </View>
...
const RequestsScreen = () => {
  ...
  const fetchRequests = async (page = 1, append = false) => {
    ...
    if (response.success) {
      setRequests(append ? prev => [...prev, ...response.data] : response.data);
  ...
  const clearFilter = () => router.replace('/(tabs)/request');
  ...
  const headerTitle = activeStateName || t('requests.title');
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyTitle}>
          {isShortSearch ? t('search.minCharsTitle', 'Keep Typing...') : t('requests.noRequests')}
        </Text>
        <Text style={styles.emptySubtitle}>
          {isShortSearch
            ? t('search.minCharsDesc', 'Enter at least 3 characters to search')
            : hasManualFilters ? t('incidents.adjustFilters') : t('requests.noRequestsDesc')}
        </Text>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
        ...
        {showSearch ? (
          ...
          <>
            ...
            <View style={styles.headerIcons}>
              <TouchableOpacity style={styles.headerIcon} onPress={() => router.push('/map-view?type=request')}>
      ...
      {canCreateRequests() && (
        <TouchableOpacity style={styles.fab} onPress={() => router.push('/add-request')} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  filterBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F3E8FF', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, marginRight: 8 },
  ...
  fab: { position: 'absolute', width: 56, height: 56, borderRadius: 28, backgroundColor: COLORS.accent, justifyContent: 'center', alignItems: 'center', bottom: 120, right: 20, ...Platform.select({ ios: { shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 }, android: { elevation: 8 } }) },
