      ...
      {transition && (
        <View style={styles.transitionBadge}>
          <Text style={styles.transitionBadgeText}>
            {transition.from_state?.name || t("common.na")} →{" "}
            {transition.to_state?.name || t("common.na")}
          </Text>
        </View>
      )}
    </View>
...
const IncidentDetailsScreen = () => {
  ...
  const getHistoryById = (historyId?: string) =>
    history.find((h: any) => h.id === historyId);
  const isDefaultLocation =
    incident?.location?.name?.trim().toLowerCase() === "default";
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      if (detailsResponse.success) {
        ...
        crashLogger
          .logWarning("Failed to fetch incident details", {
        ...
        CustomAlert.alert(
          t("common.error"),
          `${t("details.fetchError")}: ${detailsResponse.error}`,
        );
      }
      ...
      if (transitionsResponse.success) {
        const executableTransitions = transitionsResponse.data.filter(
          (t: TransitionData) => t.can_execute,
        );
  ...
  const handleUpdateStatusPress = () => {
    ...
    if (isDefaultLocation) {
      CustomAlert.alert(
        ...
        t(
          "incidents.defaultLocationTransitionBlocked",
          "Transition is not allowed while the incident location is Default. Incident location should be updated first.",
        ),
      );
      return;
    }
    ...
    router.push({
      ...
      params: {
        ...
        incident: JSON.stringify({
          id: incident.id,
          classification_id:
            incident.classification_id || incident.classification?.id,
  ...
  const viewerRoles = (
    process.env.EXPO_PUBLIC_VIEWER_APP_ROLES || "viewer,viewer2"
  ).split(",");
  const isViewerRole =
    user?.roles?.some((role) => viewerRoles.includes(role.code)) ?? false;
  ...
  const config = priorityConfig[incident.priority as number] || {
    key: "unknown",
    color: COLORS.text.muted,
  };
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
              ...
              {incident.current_state && (
                <View style={styles.statusContainer}>
                  ...
                  <Text style={styles.statusText}>
                    {i18n.language === "en" || !incident.current_state?.name_ar
                      ? incident.current_state?.name
                      : incident.current_state?.name_ar}
                  </Text>
                </View>
              )}
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { id: incident.converted_request_id },
                })
              }
            >
              <View
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  ...
                  params: { id: incident.source_incident_id },
                })
              }
            >
              <View
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    {t(
                      "incidents.convertedFromIncident",
        ...
        <AnimatedListItem index={nextSection()}>
          <View style={styles.card}>
            <SectionHeader
              title={t("incidents.incidentDetails")}
            ...
            <View style={styles.infoContainer}>
              ...
              <InfoRow
                ...
                value={
                  getPath(classTree, incident.classification?.id) ||
              ...
              <InfoRow
                ...
                value={
                  getPath(deptTree, incident.department?.id) ||
              ...
              {(() => {
                ...
                const visibleAssignees = showAllAssignees
                  ? assigneesList
                  : assigneesList.slice(0, 4);
                const hiddenCount =
                  assigneesList.length - visibleAssignees.length;
                        ...
                        styles.infoRow,
                        {
                          borderBottomColor:
                            assigneesList.length > 4
                              ? "transparent"
                              : COLORS.border,
                          paddingBottom: assigneesList.length > 4 ? 0 : 12,
                        },
                      ]}
                    >
                      <View style={styles.infoRowLeft}>
                    ...
                    {assigneesList.length > 4 && (
                      ...
                      >
                        <Text style={styles.seeMoreText}>
                          {showAllAssignees
                            ? t("common.showLess")
                            : `${t("common.viewAll")} (+${hiddenCount})`}
                        </Text>
                        <Ionicons
                          name={
                            showAllAssignees ? "chevron-up" : "chevron-down"
                          }
                          size={16}
                          color={COLORS.accent}
                        />
              ...
              {incident.location && (
                <InfoRow
                  ...
                  value={
                    getPath(locTree, incident.location.id) ||
              ...
              {incident.source && (
                <InfoRow
                  ...
                  value={t(
                    `incidents.sources.${incident.source?.toLowerCase()}`,
                  )
                    .replace(/_/g, " ")
              ...
              {incident.lookup_values &&
                incident.lookup_values.length > 0 &&
                (() => {
                  ...
                  try {
                    ...
                    Object.entries(customFields).forEach(
                      ([key, fieldData]: [string, any]) => {
                        const ld = categories.find(
                          (c: any) => c.id === fieldData.category_id,
                        );
...
<body>
  ...
  <script>
    ...
    map.whenReady(function() {
      window.ReactNativeWebView.postMessage(JSON.stringify({
                ...
                <View style={styles.mapFooter}>
                  {incident.address && (
                    <View style={styles.addressContainer}>
                      <Ionicons
                        ...
                        color={COLORS.error}
                      />
                      <Text style={[styles.addressText, { textAlign: "left" }]}>
                        {incident.address}
                      </Text>
                    </View>
                  )}
                  ...
                  >
                    ...
                    <Text style={styles.directionsButtonText}>
                      {t("details.directions") || "Directions"}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
            ...
            <View style={styles.reporterCard}>
              <View style={styles.reporterAvatar}>
                <Text style={styles.reporterAvatarText}>
                  {incident.reporter?.first_name?.[0] ||
                    incident.reporter?.username?.[0] ||
                    "U"}
                </Text>
              </View>
              ...
              <View style={styles.reporterActions}>
                {(incident.reporter_email || incident.reporter?.email) && (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      Linking.openURL(
                        `mailto:${incident.reporter_email || incident.reporter?.email}`,
                      )
                    }
                  >
                ...
                {incident.reporter?.phone && (
                  <TouchableOpacity
                    style={styles.reporterActionButton}
                    onPress={() =>
                      Linking.openURL(`tel:${incident.reporter?.phone}`)
                    }
                  >
                ...
                {incident.comments.length > 1 && (
                  ...
                  >
                    <Text style={styles.seeMoreText}>
                      {showAllComments
                        ? t("common.showLess")
                        : `${t("common.viewAll")} (${incident.comments.length})`}
                    </Text>
                    <Ionicons
        ...
        <AnimatedListItem index={nextSection()}>
          <View
            style={[
              styles.card,
              { marginBottom: availableTransitions.length > 0 ? 100 : 30 },
            ]}
          >
            <SectionHeader
              title={t("details.transitionHistory")}
              icon="git-compare"
            />
            {history && history.length > 0 ? (
              <View style={styles.timeline}>
                {(showAllHistory ? history : [history[0]]).map(
                  (item, index) => (
                    <View key={item.id} style={styles.timelineItem}>
                      <View style={styles.timelineLeft}>
                        ...
                        {(showAllHistory
                          ...
                          : false) && <View style={styles.timelineLine} />}
                      </View>
                      <View style={styles.timelineContent}>
                        ...
                        >
                          <Text style={{ fontWeight: "bold", fontSize: 12 }}>
                            {item.transition?.name ||
                              t("common.stateChanged", "State Changed")}
                          </Text>
                          <Text style={styles.transitionMeta}>
                            ...
                            {item?.performed_by?.username ||
                              ...
                              t("common.system")}{" "}
                            •{" "}
                            {new Date(item.transitioned_at).toLocaleDateString(
                              "en-GB",
                            )}
                          </Text>
                        </View>
                        <View style={styles.transitionBadges}>
                          ...
                          <View style={styles.toBadge}>
                            <Text style={styles.toBadgeText}>
                              {i18n.language === "ar" && item.to_state?.name_ar
                                ? item.to_state?.name_ar
                                : item.to_state.name}
                            </Text>
                          </View>
                        </View>
                        {item.comment && (
                          <View style={styles.transitionComment}>
                            ...
                            <RenderWithIncidentMentions
                              text={item.comment}
                              style={styles.transitionCommentText}
                            />
                          </View>
                        )}
                  ...
                  >
                    <Text style={styles.seeMoreText}>
                      {showAllHistory
                        ...
                        : `${t("common.viewAll") || "See More"} (${history.length})`}
                    </Text>
                    <Ionicons
      ...
      {/* Action Buttons */}
      <View
        style={[styles.actionButtonsContainer, { bottom: insets.bottom + 20 }]}
      >
        {availableTransitions.length > 0 && (
          <TouchableOpacity
            style={[
              styles.updateButton,
              isDefaultLocation && styles.updateButtonDisabled,
              { flex: 1 },
            ]}
        ...
        {canConvert && (
          <TouchableOpacity
            style={[
              styles.updateButton,
              isDefaultLocation && styles.updateButtonDisabled,
              { backgroundColor: COLORS.priority.high, flex: 1 },
            ]}
