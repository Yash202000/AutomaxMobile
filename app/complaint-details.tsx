...
const ComplaintDetailsScreen = () => {
  ...
  const fetchDetails = useCallback(async () => {
    ...
    if (detailsResponse.success) {
      ...
      Alert.alert('Error', `Failed to fetch complaint details: ${detailsResponse.error}`);
    }
  ...
  if (error || !complaint) {
    return <View style={styles.centered}><Text style={styles.errorText}>{error || 'Complaint not found.'}</Text></View>;
  }
        ...
        <ScrollView>
          <View style={styles.content}>
            <View style={styles.recordTypeBadge}>
              <Ionicons name="chatbubble-ellipses" size={16} color="#E74C3C" />
              <Text style={styles.recordTypeText}>Complaint</Text>
            </View>

            <Text style={styles.complaintTitle}>{complaint.title}</Text>
            <Text style={styles.complaintDate}>{new Date(complaint.created_at).toLocaleString()} ({complaint.reporter?.username || 'N/A'})</Text>
            ...
            {complaint.channel && (
              <>
                <View style={styles.detailItem}>
                  ...
                  <Text style={styles.detailValue}>{complaint.channel}</Text>
                </View>
                <DottedSeparator />
              </>
            )}
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailValue}>{complaint.classification?.name || 'N/A'}</Text>
            </View>
            <DottedSeparator />
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>Status:</Text>
              <Text style={styles.detailValue}>{complaint.current_state?.name || 'N/A'}</Text>
            </View>
            <DottedSeparator />
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>Department:</Text>
              <Text style={styles.detailValue}>{complaint.department?.name || 'N/A'}</Text>
            </View>
            <DottedSeparator />
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>Assignee(s):</Text>
              <View style={{ flex: 1, alignItems: 'flex-end' }}>
                {complaint.assignees && complaint.assignees.length > 0 ? (
                  complaint.assignees.map((assignee, index) => (
                  ...
                  <Text style={styles.detailValue}>
                    ...
                    {complaint.assignee.phone ? ` (${complaint.assignee.phone})` : ''}
                  </Text>
                ) : (
            ...
            <View style={styles.detailItem}>
              ...
              <Text style={[styles.detailValue, { flex: 1, textAlign: 'left' }]}>{complaint.description}</Text>
            </View>
            <DottedSeparator />
            ...
            {otherAttachments.map(att => (
                <TouchableOpacity key={att.id} onPress={() => Linking.openURL(`${baseURL}/attachments/${att.id}`)} style={styles.attachmentButton}>
                    <Ionicons name="attach" size={20} color="#E74C3C" />
            ...
            <View style={styles.locationContainer}>
              ...
              <View style={{ marginLeft: 10 }}>
                <Text style={styles.locationText}>{complaint.location?.name || 'No Location'}</Text>
                <Text style={styles.locationText}>{complaint.location?.address || ''}</Text>
              </View>
            </View>
            ...
            {complaint.transition_history && complaint.transition_history.length > 0 ? (
                complaint.transition_history.map(item => (
