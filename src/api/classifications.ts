...
export const getClassificationsTreeWithStats = async (type?: 'incident' | 'request' | 'complaint' | 'query' | 'both' | 'all' | 'mobile' | 'ivr') => {
  try {
    const url = type ? `/admin/classifications/tree/stats?type=${type}` : '/admin/classifications/tree/stats';
    const response = await apiClient.get(url);
    if (response.data && response.data.success) {
      return { success: true, data: response.data.data };
    }
